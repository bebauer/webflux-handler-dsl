        ...
        val router = router {
            GET("/test/{test}", handler {
                pathVariable("test".stringVar()) { test ->
                    respond {
                        ok().contentType(MediaType.APPLICATION_JSON).body(BodyInserters.fromObject(test))
                    }
                }
            })
        }
    ...
    fun `multiple mixed path variables`() {
        val router = router {
            GET("/test/{test1}/{test2}/{test3}", handler {
                pathVariables("test1".intVar(), "test2".stringVar(), "test3".stringVar()) { test1, test2, test3 ->
                    respond {
        ...
        WebTestClient.bindToRouterFunction(router)
            ...
            .returnResult().apply { Assertions.assertThat(responseBody).containsExactly(123, "a", "b") }
    }
}